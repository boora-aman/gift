  ...
  <div class="min-h-screen bg-gray-50">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        <h3 class="text-lg font-medium text-gray-900 mb-4">Search Dispatched Gifts</h3>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <!-- Gift ID Search -->
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Gift ID</label>
            <input v-model="filters.giftId" type="text" placeholder="Enter Gift ID..."
          ...
          <div>
            ...
            <input v-model="filters.giftName" type="text" placeholder="Enter Gift Name..."
              class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-gray-500 focus:border-gray-500" />
          </div>

          <!-- Persona Details Search -->
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Persona Details</label>
            <input v-model="filters.personaDetails" type="text" placeholder="Name, Emirates ID, Phone..."
              class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-gray-500 focus:border-gray-500" />
          </div>
        </div>
        ...
        <div class="px-6 py-4 border-b border-gray-200">
          <div class="flex justify-between items-center">
            <h3 class="text-lg font-medium text-gray-900 dark:text-white">
              Dispatched Gifts ({{ totalRecords }} total)
            </h3>
            <div class="flex items-center space-x-4">
              <!-- Export Button -->
              <button @click="exportReport" :disabled="isExporting || reportData.length === 0"
                class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-gray-900 hover:bg-black disabled:opacity-50">
              ...
              <tr v-else v-for="record in reportData" :key="record.dispatch_id">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div>
                    ...
                    <div class="text-sm text-gray-500 dark:text-gray-400" v-if="record.emirates_id">{{
                      record.emirates_id }}</div>
                  </div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div>
                    ...
                    <div class="text-sm text-gray-500 dark:text-gray-400" v-if="record.address">{{ record.address }}
                    </div>
                  </div>
                </td>
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div>
                    ...
                    <div class="text-sm text-gray-500 dark:text-gray-400 mt-1" v-if="record.delivery_date">
                      Delivered: {{ formatDate(record.delivery_date) }}
                    </div>
                    <div class="text-sm text-gray-500 dark:text-gray-400 mt-1" v-if="record.delivery_note">
  ...
  methods: {
    async loadReport() {
      ...
      try {
        ...
        const result = await ReportsAPI.getDispatchedGiftsReport(params)
    ...
    async exportReport() {
      ...
      try {
        ...
        exportToExcel(result.data, columns, 'Dispatched_Gifts_Report')
